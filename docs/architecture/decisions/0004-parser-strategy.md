...
Shape provides parser infrastructure for validation schema formats (JSONV, XMLV, YAMLV, CSVV, TEXTV, PropsV). We needed to choose a parsing strategy that would:

1. Be simple and maintainable
2. Produce clear, actionable error messages
3. Support multiple validation schema formats
4. Be fast enough for production use
5. Work well with our tokenizer-based architecture

## Decision

We use **LL(1) recursive descent parsing** for all validation schema parsers.
...
**Current status:** All 10 parser projects currently use LL(1) recursive descent because it provides the best combination of simplicity, performance, and error quality for validation schemas. But the door remains open for different techniques if needed.
...
### 6. Sufficient for Validation Schemas
- Validation schema grammars are naturally LL(1)
...
**Mitigation:** Design validation schema grammars to be LL(1)-compatible from the start.
...
**Mitigation:** Validation schema formats don't need multi-token lookahead. Tokenizer can combine tokens if needed (e.g., recognize "if (" as single token).
...
**Mitigation:** Validation schemas are naturally unambiguous (types, functions, literals are distinct).
...
- Overkill for validation schemas
...
- Unnecessary for validation schemas
...
**Decision:** Rejected. Too complex for validation schemas.
...
All validation schema parsers follow this pattern:

- **shape-jsonv:** `shape-jsonv/internal/parser/jsonv/parser.go`
- **shape-xmlv:** `shape-xmlv/internal/parser/xmlv/parser.go`
- **shape-yamlv:** `shape-yamlv/internal/parser/yamlv/parser_native.go`
- **shape-csvv:** `shape-csvv/internal/parser/csvv/parser.go`
- **shape-textv:** `shape-textv/internal/parser/textv/parser.go`
- **shape-props:** `shape-props/internal/parser/propsv/parser.go`
