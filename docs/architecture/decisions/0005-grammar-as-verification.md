...
Shape provides parser infrastructure for 10 parser projects (JSONV, XMLV, YAMLV, CSVV, TEXTV, PropsV, and data format validators). We use hand-coded parsers for maximum performance and error quality (see ADR 0004).
...
- Test parser projects (shape-jsonv, shape-xmlv, etc.)
- Know about specific format grammars
- Have dependencies on parser projects

### Parser Project's Responsibility (Self-Contained)

**Each parser project (shape-jsonv, shape-xmlv, etc.):**
- Maintains its own EBNF grammar (`docs/grammar/jsonv.ebnf`)
...
**Example: shape-jsonv tests itself**
```go
// shape-jsonv/grammar_test.go
...
func TestGrammarVerification(t *testing.T) {
    ...
    spec := grammar.ParseEBNF("docs/grammar/jsonv.ebnf")
...
│                                                     │
│  internal/parser/jsonv/parser.go                    │
...
grammar := ParseEBNF("docs/grammar/jsonv.ebnf")
...
```
Task: Implement a parser for the JSONV validation schema language.

Inputs:
1. EBNF Grammar: docs/grammar/jsonv.ebnf (with implementation hints)
...
```go
// Package parser implements the JSONV validation schema parser.
//
...
When grammar changes:
1. Update grammar file (`docs/grammar/jsonv.ebnf`)
...
```bash
$ git diff docs/grammar/jsonv.ebnf
...
**New contributor asks: "What does JSONV accept?"**

**With enforced grammar:**
- "Read `docs/grammar/jsonv.ebnf`"
...
**Shape has 10 parser projects. Grammar shows consistency:**

```ebnf
// shape-jsonv/docs/grammar/jsonv.ebnf
Value = Literal | Type | Function | Object | Array ;

// shape-yamlv/docs/grammar/yamlv.ebnf
Value = Literal | Type | Function | Object | Array ;  # Same!

// shape-xmlv/docs/grammar/xmlv.ebnf
...
├── docs/
│   └── grammar/
│       └── jsonv.ebnf              # Canonical specification
├── internal/
│   └── parser/
│       └── jsonv/
...
**Development:**
1. Define grammar: `docs/grammar/jsonv.ebnf`
2. Implement parser: `internal/parser/jsonv/parser.go`
3. Generate tests: `go run tools/grammar-test-gen docs/grammar/jsonv.ebnf > internal/parser/jsonv/grammar_test.go`
...
**Used by all 10 parser projects:**
```go
import "github.com/shapestone/shape/pkg/grammar"

func TestGrammar(t *testing.T) {
    grammar := grammar.ParseEBNF("docs/grammar/jsonv.ebnf")
...
- **README references grammar** - "See docs/grammar/jsonv.ebnf for specification"
