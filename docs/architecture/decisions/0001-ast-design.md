...
Shape needs a unified Abstract Syntax Tree (AST) representation for validation schemas across 6 different formats (JSONV, XMLV, PropsV, CSVV, YAMLV, TEXTV). The AST must be:

1. **Format Agnostic:** Same AST structure regardless of input format
2. **Complete:** Represent all validation constructs
3. **Traversable:** Easy to walk and validate against data
4. **Serializable:** Can be saved/loaded for caching
5. **Extensible:** Support future validator types
...
- AST can represent all validation constructs from all 6 formats
- Visitor pattern enables data validation traversal
