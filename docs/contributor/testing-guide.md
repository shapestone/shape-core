...
func TestParse(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            got, err := Parse(parser.FormatCustom, tt.input)
...
func TestParse_ErrorCases(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            _, err := Parse(parser.FormatCustom, tt.input)
...
```go
func TestCustomParser_ParseObject(t *testing.T) {
    parser := NewParser()
...
func BenchmarkParse_Simple(b *testing.B) {
    ...
    b.ResetTimer()
    for i := 0; i < b.N; i++ {
        _, err := shape.Parse(parser.FormatCustom, input)
        if err != nil {
            b.Fatal(err)
        }
    }
...
func TestCrossFormat_SameAST(t *testing.T) {
    ...
    // Note: This test would be in a parser project that supports multiple formats
    schemas := map[parser.Format]string{
        parser.FormatCustom1: `{"id": UUID}`,
        parser.FormatCustom2: "id: UUID",
        parser.FormatCustom3: "id=UUID",
    }
